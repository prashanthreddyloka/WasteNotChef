...
export function Fridge({ items, onUpdateItem, onRemoveItem, onAddItem, onGenerateRecipes, generatingRecipes, onItemsAdded, synced }: FridgeProps) {
    ...
    <div className="pantry-page space-y-7">
      ...
      <div className="pantry-tools"><ReceiptUpload onItemsAdded={onItemsAdded} /><section className="manual-entry"><p className="eyebrow">JUST ONE MORE THING</p><h2>Add it yourself.</h2><p>Something missing? Make a little room for it here.</p>
        <form onSubmit={async event => {
          ...
          try { await onAddItem({ id: `manual-${crypto.randomUUID()}`, name: draftName.trim(), quantity: draftQuantity.trim() || undefined, detectedExpiry: selectedExpiry || null, inferredExpiry: null, confidence: 1, detectionSource: "manual", expirySource: selectedExpiry ? "manual" : "none", reviewed: true, notes: "Added manually." });
          setMessage(`${draftName.trim()} added to your pantry.`); setDraftName(""); setDraftQuantity(""); setDraftExpiry("");
          } catch { /* Keep the form intact so a failed sync can be retried. */ }
      ...
      {removed && <div className="undo-banner" role="status"><span>{removed.name} removed.</span><button type="button" onClick={async () => { try { if (!items.some(item => item.id === removed.id)) await onAddItem(removed); setMessage(`${removed.name} restored.`); setRemoved(null); } catch {} }}>Undo removal</button></div>}
      <p className="sr-only" role="status">{message}</p>
      {!visible.length ? <div className="pantry-empty"><span aria-hidden="true">✳</span><h2>{items.length ? "Nothing on this shelf yet." : "Your next good meal starts here."}</h2><p>{items.length ? "Try a different ingredient name." : "Scan a receipt or add your first ingredient above."}</p></div> : <div className="pantry-grid">{visible.map(item => <PantryCard key={item.id} item={item} synced={synced} onSave={async updates => { await onUpdateItem(item.id, updates); setMessage(`${updates.name ?? item.name} saved.`); }} onRemove={async () => { await onRemoveItem(item.id); setRemoved(item); setMessage(`${item.name} removed.`); }} />)}</div>}
      <p className="pantry-storage-note">{synced ? "Your edits sync with your account." : "Your edits are saved on this device."} Estimated dates are a guide; always check the packaging.</p>
    </div>
  );
}
